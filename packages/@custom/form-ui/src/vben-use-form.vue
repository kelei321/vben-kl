...
import { buildDefaultValues } from './zod/build-default-values';
...
const { delegatedSlots } = useDelegatedSlots();

const defaultValues = computed(() =>
  buildDefaultValues(forward.value.schema ?? []),
);
const formSchema = computed(() => buildZodSchema(forward.value.schema ?? []));

const form = useForm({
  defaultValues: defaultValues.value,
  validators: {
    onSubmit: formSchema.value,
  },
  onSubmit: async () => {
    const values = await props.formApi?.getValues();
    props.formApi?.setLatestSubmissionValues(values ?? {});
    await forward.value.handleSubmit?.(values ?? {});
  },
});

// TanStack Form state is granularly reactive. Plain `form.state.values` is not
// enough for Vue watchers in nested custom renderers, so subscribe explicitly.
const formValues = form.useStore((formState) => formState.values);

watch(
  () => forward.value.schema,
  () => {
    form.update?.({
      defaultValues: defaultValues.value,
      validators: {
